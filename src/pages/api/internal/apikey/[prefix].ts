...
function validPrefix(prefix: string | string[] | undefined): boolean {
...
export default async function handler(
  ...
  if (!validPrefix(prefix)) {
    return res.status(400).json({ error: "Missing or invalid key prefix" });
  }

  try {
    const config = getHeadscaleConfig(req);
    const response = await headscaleFetch(`/api/v1/apikey/${prefix}`, {
      method: "DELETE",
    }, config);
    ...
    if (!response.ok) {
      console.error("Headscale delete apikey error:", response.status, data);
      return res
        .status(502)
        .json({
          error:
