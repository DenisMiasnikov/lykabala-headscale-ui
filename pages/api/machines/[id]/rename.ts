...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const renameRes = await headscaleFetch(`/api/v1/node/${id}/rename/${name}`, {
      method: "POST"
    });
    ...
    if (!renameRes.ok) {
      ...
      return res.status(502).json({ error: data?.message || data?.error || `Headscale error ${renameRes.status}` });
    }
