...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const { id } = req.query;
  if (!validId(id)) return res.status(400).json({ error: "Invalid namespace ID" });
  ...
  if (req.method === "DELETE") {
    try {
      ...
      if (!response.ok) {
        ...
        return res.status(502).json({ error: data.error || "Failed to delete namespace" });
      }
  ...
  if (req.method === "GET") {
    ...
    return res.status(501).json({ 
      error: "Headscale API does not support getting individual namespace. Use the list endpoint instead." 
    });
  }
