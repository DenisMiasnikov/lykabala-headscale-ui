          ...
          <div style={{ marginBottom: 24, padding: 16, background: "#f5f5f5", borderRadius: 8 }}>
            ...
            <div style={{ display: "flex", gap: 8, alignItems: "center", marginTop: 8 }}>
              <input
                ...
                style={{ flex: 1 }}
              />
              <button className="button" onClick={createKey} disabled={creating}>
              ...
              <tbody>
                {keys.map((k) => (
                  <tr key={k.prefix}>
                    ...
                    <td style={{ padding: 8 }}>{k.expiration ? new Date(k.expiration).toLocaleString() : "-"}</td>
                    <td style={{ padding: 8 }}>{k.lastSeen ? new Date(k.lastSeen).toLocaleString() : "-"}</td>
                    <td style={{ padding: 8 }}>
                      <button className="button secondary" onClick={() => deleteKey(k.prefix)}>
                        Revoke
                      </button>
                    </td>
                  </tr>
...
export const getServerSideProps: GetServerSideProps<ApiKeysPageProps> =
  async (context) => {
